...
OUTPUT_FILE = "cleaned_entities.json"
...
def clean_slug(text):
    if not text: return ""
    t = text.lower()
    # CAMBIO: Reemplaza espacios, puntos Y GUIONES por barra baja
    t = re.sub(r'[\s\-\.]+', '_', t)
...
def process_image(item_name, original_image_name_json):
    ...
    slug = clean_slug(item_name) # Ahora genera ej: "ash_prime"
    if not slug: return None

    dest_dir = os.path.join(IMG_DIR, ENTITIES_SUBDIR)
    ...
    # 2. Busca con el slug antiguo (guion) por si la imagen origen aun no se renombró
    ...
    # Búsqueda laxa si no encuentra exactos
    if not found_src:
    ...
    try:
        # El destino SIEMPRE usa el slug con barra baja
        dest_filename = f"{slug}.webp"
        dest_path = os.path.join(dest_dir, dest_filename)

        if os.path.exists(dest_path):
            return f"{ENTITIES_SUBDIR}/{dest_filename}"

        with Image.open(found_src) as img:
            img.thumbnail(MAX_SIZE)
            img.save(dest_path, "WEBP", quality=WEBP_QUALITY, optimize=True)
            print(f"[OPTIMIZADO] {os.path.basename(found_src)} -> {ENTITIES_SUBDIR}/{dest_filename}")
            return f"{ENTITIES_SUBDIR}/{dest_filename}"
        ...
        try:
            ...
            with urllib.request.urlopen(req) as response:
                ...
                for item in data:
                    ...
                    all_entities.append(cleaned_item)
