 ...
 * @param {boolean} [force] Ignora TODAS las cachés (IndexedDB y la del navegador/edge).
 *   Lo usa el refetch de rotación: con el suelo de 60s del expiryTime, la entrada recién
 *   guardada sigue vigente y sin esto el reintento se respondía a sí mismo con las
 *   misiones caducadas.
...
export async function fetchActiveBounties(force = false) {
    try {
        ...
        // force viaja hasta la petición: saltarse solo IndexedDB no sirve de nada si el
        // navegador responde con su propia copia del ciclo anterior (ver getActiveBounties).
        const res = await getActiveBounties(force);
        if (!res.ok) return [];
