...
/**
 * Reparte un evento entre sus suscriptores sin que uno roto se lleve a los demás.
 *
 * Mismo criterio que wfm_socket.service.js, que ya lo hacía: aquí llegan ~250 órdenes por
 * minuto y una excepción de un listener se la tragaba el socket, así que el síntoma no era un
 * error sino avisos que dejaban de sonar sin motivo aparente.
 */
function emitir(listeners, evento) {
...
function handleOrder(order) {
    ...
    if (better) {
        ...
        emitir(priceListeners, { itemId, name: info.name, ...next });
    }
