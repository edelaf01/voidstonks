...
import { fetchRivenAverage } from "../../services/rivens/rivens.service.js";
import { getMetaStats, fetchSimilarRivens } from "../../services/rivens/riven_market.service.js?v=1.9";
import {
  getItemIcon,
  DEFAULT_WEAPON_SVG,
...
function switchGradingRoll(index) {
  ...
  try {
    ...
    if (hasPremiumData) {
      ...
      if (cached && cached.rivenData && Object.keys(cached.rivenData).length > 0) {
        ...
        // 3. Fetch from worker
        try {
          const indice = await getRivenIndex();
          if (indice.ok) {
            if (indice.baseline) state.rivenStatBaseline = indice.baseline;
            const merged = { ...(globalThis.dynamicMetaStats || {}) };
            for (const [key, val] of Object.entries(indice.weapons)) {
              if (merged[key]) {
...
// Los tres los invoca markup con onclick/onchange: `changeHistoryRange` desde index.html, y los
// otros dos desde HTML que genera este mismo módulo.
exposeGlobals({
  changeHistoryRange,
  switchGradingRoll,
  loadMoreRivenIndex,
}, "ui.components/rivens/ui_rivens.js");
