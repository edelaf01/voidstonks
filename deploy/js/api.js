...
export function getRivenSlug(inputVal) {
  ...
  // Solo quitamos el prefijo/sufijo si existe la versión base en la base de datos.
  // Ejemplo: Nikana Prime -> Existe Nikana -> slug = nikana
  // Ejemplo: Kuva Bramma -> NO existe Bramma -> slug = kuva_bramma
...
export async function fetchRivenWeapons() {
  const CACHE_KEY = "voidstonkscache_fix_v2"; // Bumped version to force refresh
  const ONE_DAY = 24 * 60 * 60 * 1000;

  try {
    const cached = await dbHelper.get(CACHE_KEY);

    // Add timestamp validation (expire after 24h)
    if (cached?.data && cached?.timestamp && (Date.now() - cached.timestamp < ONE_DAY) && Object.keys(cached.data).length > 100) {
      console.log(
    ...
    if (typeof rawWeapons === "object" && !Array.isArray(rawWeapons)) {
      Object.keys(rawWeapons).forEach((name) => {
        ...
        const realDispo = item.omegaAttenuation || item.d || 1;
        const realType = item.type || item.t || "Rifle";

        finalMap[name] = {
          d: Number.parseFloat(realDispo),
          t: realType,
        };
      });
      ...
      rawWeapons.forEach((item) => {
        ...
        finalMap[name] = {
          d: Number.parseFloat(item.omegaAttenuation || item.d || 1),
    ...
    console.error("Error cargando armas:", e);
  }
}
...
async function processQueue() {
  ...
  // Split slugsToFetch into chunks of 25 to avoid URL length & worker limits
...
export async function fetchPrimeManifest() {
  try {
    const res = await fetch("assets/json/warframe_prime_clean.json");
    if (!res.ok) throw new Error("Manifest Load Failed");
    const data = await res.json();
    state.primeManifest = data;
    console.log("Prime Manifest Loaded:", data.length, "items");
  } catch (e) {
    console.warn("Error loading prime manifest:", e);
  }
}

export async function fetchRelicDatabase() {
  try {
    const res = await fetch("assets/json/database_reliquias.json");
    if (!res.ok) throw new Error("Database Load Failed");
    state.ducatsDatabase = await res.json();
    console.log("Relic Database Loaded");
  } catch (e) {
    console.warn("Error loading relic database:", e);
  }
}
...
export async function fetchActiveBounties() {
  ...
  try {
    ...
    const res = await fetch(`${WORKER_URL}?type=active_bounties&t=${now}`);
    if (!res.ok) return [];
